...
export const JobApplicationsManager = memo(({ jobId, onClose }) => {
  ...
  const loadApplications = async () => {
    try {
      setLoading(true);
      const response = await jobApi.getApplications(jobId);
  ...
  const getStatusConfig = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case 'interview':
      case 'phone_screen':
        return { color: 'bg-cyan-500/20 text-cyan-500', icon: MessageSquare, label: status.toUpperCase() };
      case 'accepted':
                          ...
                          {(application.status?.toLowerCase() === 'reviewed' || 
                            ...
                            <>
                              <Button
                                ...
                                disabled={updating === application.id}
                                className="font-mono text-xs bg-cyan-500/10 hover:bg-cyan-500/20 border-cyan-500/30"
                              >
                                {updating === application.id ? (
                                  <LoadingSpinner size="sm" />
                                ) : (
...
JobApplicationsManager.displayName = 'JobApplicationsManager';
